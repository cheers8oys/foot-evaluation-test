...
시즈탱크 발 유형 테스트 MVP에서는 사용자가 8문항 테스트를 완료하고 휴대폰 번호를 제출하면, 신규 번호에 한해 문자온 알림톡 API로 결과지 링크를 발송한다.
...
```json
{
  ...
  "callFrom": "01012345678",
  "callTo_1": "01098765432",
...
export function buildFallbackSms(input: {
  caseName: string;
  resultUrl: string;
}) {
...
3. answers로 결과 재계산
4. 전화번호 정규화
5. Google Sheets에서 중복 번호 확인
6. 기존 번호면 duplicate 응답
7. 신규 번호면 Google Sheets row 추가
8. sendAlimtalk 호출
9. 발송 성공이면 created 응답
10. 발송 실패면 MESSAGE_SEND_FAILED 응답
```
...
| 케이스 | 기대 결과 |
|---|---|
| 신규 번호 + 테스트 성공 | Google Sheets 저장, 알림톡 성공 응답 |
...
- 특히 `callTo_1`, `templateContent_1`, `subMsgTxt_1`에는 개인정보와 결과 URL이 포함될 수 있다.
