...
시즈탱크 발 유형 테스트는 모바일 중심의 8문항 A/B 테스트 서비스다. 사용자는 문항에 답한 뒤 이름과 휴대폰 번호를 입력하고 단일 동의 체크박스를 선택한다. 서버는 제출 시간이 너무 빠른 요청과 중복 전화번호를 차단하고, 신규 사용자만 Google Sheets에 저장한 뒤 알림톡 또는 SMS로 결과지 링크를 발송한다.
...
6. `/quiz/contact`에서 이름과 휴대폰 번호를 입력한다.
7. 단일 동의 체크박스를 선택한다.
...
| P-04 | `/quiz/contact`   | page | 이름, 휴대폰 번호 입력 및 동의 화면 |
...
| 항목        | 필수 | 저장 | 설명                       |
| ----------- | ---- | ---- | -------------------------- |
| 이름        | 예   | 예   | CRM 식별 및 상담 응대용    |
| 휴대폰 번호 | 예   | 예   | 결과지 발송 및 후속 안내용 |

### 10.2 이름 형식

- 입력 UI는 공백 없는 실명 또는 호칭 입력을 유도한다.
- 서버 저장 시 앞뒤 공백을 제거한다.
- 허용 형식은 trim 후 1~30자 문자열이다.
- 서버는 빈 문자열이거나 30자를 초과한 이름을 저장하지 않는다.

### 10.3 전화번호 형식

- 입력 UI는 `010-XXXX-XXXX` 형태를 유도한다.
- 서버 저장 시 숫자만 남겨 `<NORMALIZED_PHONE>` 형태로 정규화한다.
...
전화번호 형식 오류는 `INVALID_PHONE`, 미동의는 `CONSENT_REQUIRED` 등 `11.3 서버 검증 규칙`의 실패 코드와 메시지를 따른다.
...
| 검증 항목                     | 실패 코드             | 처리 |
| ----------------------------- | --------------------- | ---- |
| 이름 누락 또는 형식 오류      | `INVALID_NAME`        | 400  |
...
분석 이벤트에는 이름, 휴대폰 번호 등 개인정보를 포함하지 않는다.
...
- 이름은 MVP에서 trim된 평문으로 저장한다.
- 전화번호는 MVP에서 정규화된 평문으로 저장한다.
- 서버는 이름, 전화번호, 동의 여부, 토큰, 답변, 결과를 모두 검증한다.
...
- 신규 번호는 이름과 함께 Google Sheets에 1회 저장된다.
